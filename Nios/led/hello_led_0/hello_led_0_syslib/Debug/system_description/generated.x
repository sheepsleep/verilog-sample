 ...
 * d:\2.3\Nios\led\hello_led_0\hello_led_0_syslib\..\..\nios_ucos.ptf
 *
 * Generated: 2011-10-01 00:21:13.546
 *
 */
...
MEMORY
{
    reset : ORIGIN = 0x00008000, LENGTH = 32
    onchip_ROM : ORIGIN = 0x00008020, LENGTH = 2016
    onchip_RAM_UNUSED : ORIGIN = 0x00010000, LENGTH = 32
    onchip_RAM : ORIGIN = 0x00010020, LENGTH = 992
}

    /* Define symbols for each memory base-address */
 __alt_mem_onchip_ROM = 0x00008000 ;
 __alt_mem_onchip_RAM = 0x00010000 ;
...
SECTIONS
{
    ...
    .text :
    {
        ...
        *(.dynamic)
        ...
        KEEP (*(.jcr))
        . = ALIGN(32 / 8);
    } >  onchip_RAM =0x3a880100 /* NOP on Nios2 (big endian) */
    ...
    .onchip_ROM :
    {
        PROVIDE (_alt_partition_onchip_ROM_start = ABSOLUTE(.));
        *(.onchip_ROM .onchip_ROM.*)
        . = ALIGN(32 / 8);
        PROVIDE (_alt_partition_onchip_ROM_end = ABSOLUTE(.));
    } > onchip_ROM

    PROVIDE (_alt_partition_onchip_ROM_load_addr = LOADADDR(.onchip_ROM));
...
PROVIDE( __alt_heap_limit    = 0x00010400 );
