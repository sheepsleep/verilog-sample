...
module hc164_driver(
  ...
  always @ ( posedge clk or negedge rst_n )
    if (!rst_n ) 
         seg_led_num <= 3'b000;
    else if ( clk_cnt == 15'h7FFF ) 
         seg_led_num <= seg_led_num + 1'b1;
  ...
  always @ ( posedge clk or negedge rst_n )
    ...
    else if ( clk_cnt[14] ) tx_cnt <= 6'd0;      
    else if ((!clk_cnt[14]) && (tx_cnt <= 6'd32 )) tx_cnt <= tx_cnt + 1'b1;

  always @ ( posedge clk or negedge rst_n )
    if (!rst_n)  hc_cp <= 1'b0;
    else if ( clk_cnt[14] ) hc_cp <= 1'b0;
    else if ((!clk_cnt[14]) && (tx_cnt < 6'd32 )) hc_cp <= !hc_cp;
